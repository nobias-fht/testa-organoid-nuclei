...
#         print('starting preprocessing for channel ' + str(channel))
#         if channel == dapi_channel:
#             skimage.io.imsave(raw_savefolder + os.path.sep + 'channel_dapi' + os.path.sep + filename[:-4] + '.tif', im[channel,:,:].astype(np.uint16), check_contrast=False)
#             print('finished saving DAPI raw')
#         else:
#             skimage.io.imsave(raw_savefolder + os.path.sep + 'channel_' + str(channel) + os.path.sep + filename[:-4] + '.tif', im[channel,:,:].astype(np.uint16), check_contrast=False)
#             print('finished saving raw')

#             sub_im = tophat_process(im[channel,:,:].astype(np.uint16))
#             print('finished subtraction')

#             skimage.io.imsave(bg_sub_folder + os.path.sep + 'channel_' + str(channel) + os.path.sep + filename[:-4] + '.tif', sub_im, check_contrast=False)
...
#         else:
#             os.makedirs(raw_image_folder + os.path.sep + 'channel_' + str(i), exist_ok=True)
#             os.makedirs(bg_sub_folder + os.path.sep + 'channel_' + str(i), exist_ok=True)
